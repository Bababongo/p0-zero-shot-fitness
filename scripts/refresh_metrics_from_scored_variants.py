...
from p0_zero_shot_fitness.pipeline import load_residue_groups
...
def load_scored_variants(path: Path, residue_groups: dict[str, set[int]]) -> list[VariantRecord]:
        ...
        for row in csv.DictReader(handle):
            ...
            records.append(
                VariantRecord(
                    ...
                    residue_groups=frozenset(row_groups | computed_groups),
                )
            )
    return records
...
def main() -> int:
    ...
    records = load_scored_variants(args.scored_variants_csv, residue_groups)
